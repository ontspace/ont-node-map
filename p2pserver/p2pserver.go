...
func (this *P2PServer) FetchAddrTo() {
	peers := this.network.GetNeighbors()
	for _, p := range peers {
		if p.GetState() == common.ESTABLISH {
			addrMsg := msgpack.NewAddrReq()
			go this.Send(p, addrMsg, false)
		}
	}
}
...
func (this *P2PServer) tryRecentPeersNew() {
	nodes := storage.ListAllNodes()
	for _, node := range nodes {
		if node.Height > 0 {
			addr := fmt.Sprintf("%s:%d", node.Ip, node.Port)
			go this.network.Connect(addr)
		}
	}
}
