...
func (this *NetServer) init() error {
	...
	log.Info("fyz", config.DefConfig.P2PNode.NodePort)
...
func (this *NetServer) Connect(addr string) error {
	...
	if connCount >= config.DefConfig.P2PNode.MaxConnOutBound {
		log.Infof("[p2p]Connect: out connections(%d) reach the max limit(%d)", connCount,
	...
	if added := this.AddOutConnectingList(addr); added == false {
		log.Info("[p2p]node exist in connecting list", addr)
	}
	this.connectLock.Unlock()
	...
	if isTls {
		...
		if err != nil {
			...
			log.Infof("[p2p]connect %s failed:%s", addr, err.Error())
			return err
		}
	} else {
	...
	log.Infof("[p2p]peer %s connect with %s with %s",
	...
	version := msgpack.NewVersion(this, 4555000)
