...
func AddrReqHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	log.Info("[p2p]receive addr request message", data.Addr, data.Id)
...
func PingHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	if data.Addr == "52.32.60.120:20338" {
		log.Info("Get Ping From Remote", data.Addr, ping.Height)
	}
	//log.Info("Get Ping From Remote", data.Addr, ping.Height)
...
func PongHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	storage.UpdateNodeHeight(remotePeer, pong.Height)
...
func VersionHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	if s == msgCommon.INIT {
		...
		msg = msgpack.NewVersion(p2p, 100000)
	...
	log.Debug("Version: ", data.Addr, version.P.Version, version.P.Services, version.P.HttpInfoPort, version.P.SoftVersion)
	storage.AddOrUpdateNodeAfterReceiveVersionMsg(remotePeer, version.P, remotePeer.GetHttpInfoState())
...
func VerAckHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	storage.AddOrUpdateNodeAfterReceiveVersionAckMsg(remotePeer)
...
func AddrHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	log.Info("[p2p]handle addr message", data.Addr, data.Id)
	...
	for _, v := range msg.NodeAddrs {
		...
		storage.TryAddNodeAfterReceiveAddrMessage(address, v.Services, uint64(v.Time))
